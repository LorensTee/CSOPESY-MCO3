...
**Established:** 2026-09-22 · **Last updated:** 2026-09-24 (**W2: Phase 1D `T2.1`–`T2.6` landed** — parameters, CLI, line editor, interpreter, entry path + `ctest smoke`, `README.txt`; **no frozen header touched** (11/11 hashes), guard `OK (22 files scanned)`, `ctest` 2/2 (unit 88 + smoke). Prior: W3 `T3.1`/`T3.2` and `T1.1`–`T1.3` remain CI-green.) 
...
| **W2 Phase 1D — DONE 2026-09-24** | ✅ **`T2.1`–`T2.6` implemented, TDD red-before/green-after per task.** `parameters.cpp` (clamp + `ClampReport`; `setText` trim/internal-run → Ok/Empty/NonAscii), `cli.cpp` (three flags; unknown/malformed warn+continue, out-of-range clamps+reports, last-wins), `line_editor.cpp` (`feed` echo/Backspace/Enter-Eof/ignored arrows + `visibleSlice` tail window), `interpreter.cpp` (case-sensitive dispatch table, exact §3.7 lines, `[-+]?[0-9]+` full match, three `set_text` outcomes), `main.cpp` + `console_app.cpp` (RAII raw-mode guard, initial frame before `start()`, input/command loop, `requestStop`→`join`→goodbye order, `--no-tty` plain line mode with **no worker**) + the portable `ctest smoke` real-binary test, `README.txt`. **`T2.4` unblocks W1's deferred T1.3 command-effect assertions; `T2.5` unblocks `T1.4` and `T3.4`.** The composition root is **locals in `run()`**, not new private members, so `console_app.hpp` keeps its v2.6 hash (risk 6 not reopened). `T2.6`'s frozen-artifact half deliberately moves to `T6.3`. Commits `14eee96`,`135a950`,`a684816`,`f4a9411`,`f0c6fab`,`71eafb8`,`89d5fe1`. |
...
- **T1.3's test suite is deliberately scheduler-only, and the deferred assertions are enumerated rather than
  faked.** `Renderer::buildFrame` (T4.2, W4) is still a stub returning `{}` and `Interpreter::feed` (T2.4, W2) is
  still a stub returning `false`, so at T1.3 time **no `Terminal::write()` ever happens** (`tick` keeps the
  ...
  the `size()`-call tick barrier. **Owed to T4.2/T2.4 — do not re-derive these from the v2/v3 prose:**
...
| **S6** | Implement Phase 1 against the re-frozen v3.0 contracts (not a documentation stage) | 🟨 **in progress — T1.1 (`PosixTerminal`) implemented and pty-verified, T1.2 (`MarqueeProcess` PCB) implemented, unit-tested, and CI-green on all three OSes (`ed18908`), both 2026-09-22; T1.3 (the threaded scheduler) landed as **`95b0b61`** — CI green on all three OSes**; T2.x, T3.1, T4.x still `TODO`. The only stage that scores points |
...
5. Implement Phase 1                              → S6   🟨 T1.1 done; T1.2 done; T1.3 next (critical path)
