...
- **`csopesy-quiz`'s hand-authored type id is WRONG for CLion 2026.2.3 — now verified, not assumed (2026-09-24).**
  The committed `.idea/runConfigurations/csopesy-quiz.xml` declares `type="CustomBuildApplication"`, but that
  id occurs **nowhere** in the installed CLion 2026.2.3 — not in any `.jar` entry (every `plugins/**` + `lib/**`
  archive scanned) and not in any plain file. The type the GUI offers as *Custom Build Application* is
  `com.jetbrains.cidr.cpp.execution.external.run.CLionExternalRunConfigurationType`
  (`intellij.clion.execution.jar`); `javap` of its constructor shows the `ConfigurationTypeBase` id is
  **`CLionExternalRunConfiguration`**, with display name bundle key `external.run.configuration.name`
  ("Custom Build Application") in `CLionExecutionBundle.properties`. Its executable is read via
  `ExecutableData.loadExternal` (flat attribute `RUN_PATH`, or a configured *Custom Build Target*), **not** the
  hand-written `EXECUTABLE` attribute — so the whole attribute set is suspect, not just the id. (`csopesy-dev`
  is fine: `javap` confirms `CMakeRunConfiguration` **is** the registered id of `CMakeAppRunConfigurationType`.)
  **Consequence:** `csopesy-quiz` must be recreated in the GUI at T3.4 before any Run press; the record and the
  exact fallback are in `docs/clion-run-config.md`, and the T6.3 pre-flight row in `docs/frozen-artifact.md`
  points at it. This is the T3.4 prerequisite the handoff §5.3 could not check because CLion ships compressed.
