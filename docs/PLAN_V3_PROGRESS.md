...
**Established:** 2026-09-22 · **Last updated:** 2026-09-22 (**T0.6 closed; Phase 1: T1.1 + T1.2 pushed and CI-green, T1.3 implemented and locally verified — commit/push pending**)
...
| **S6** | Implement Phase 1 against the re-frozen v3.0 contracts (not a documentation stage) | 🟨 **in progress — T1.1 (`PosixTerminal`) implemented and pty-verified, T1.2 (`MarqueeProcess` PCB) implemented, unit-tested, and CI-green on all three OSes (`ed18908`), both 2026-09-22; T1.3 (the threaded scheduler) implemented and locally verified 2026-09-22 — 16 tests, red-before/green-after, 150/150 clean runs, `docs/threading-model.md` written, commit/push + CI pending**; T2.x, T3.1, T4.x still `TODO`. The only stage that scores points |
