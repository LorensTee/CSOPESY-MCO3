...
**Established:** 2026-09-22 · **Last updated:** 2026-09-24 (**W2: Phase 1D `T2.1`–`T2.6` landed** — parameters, CLI, line editor, interpreter, entry path + `ctest smoke`, `README.txt`; **no frozen header touched** (11/11 hashes), guard `OK (22 files scanned)`, `ctest` 2/2 (unit 90 + smoke) — unchanged after the same-day W2 follow-up: `feed(Eof)` now requests quit (all six exit paths pty-verified, `24c41e9`) and README names Ninja + §9 comment cleanup (`ff602f4`). Prior: W3 `T3.1`/`T3.2` and `T1.1`–`T1.3` remain CI-green.) 
